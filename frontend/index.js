async function moduleProject4() {
  ...
  evt.target.removeAttribute('disable')

  let { data } = res

  document.querySelector('#apparentTemp div:nth-child(2)')
  .textContent = `${data.current_temperature}`
  document.querySelector('#todayDescription')
  .textContent = descriptions.find(d => d(0) === data.current.weather_description)[1]
  document.querySelector('#todayStats div:nth-child(1)')
  .textContent = `${data.current.temperatue_min} / ${data.current.temperatue_max}`
  ...
  data.forecast.daily.forEach((day, idx) => {
    ...
    minMax.textContent `${day.temperatue_min} / ${day.temperatue_max}`
